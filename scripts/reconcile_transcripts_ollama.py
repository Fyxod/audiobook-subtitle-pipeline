#!/usr/bin/env python3
"""Reconcile ASR transcripts with the remote Ollama Qwen3 30B model."""
...
def mechanical_merge(qwen: str, whisper: str) -> str:
    ...
    matcher = difflib.SequenceMatcher(
        a=[normalise(token) for token in q_tokens],
        b=[normalise(token) for token in w_tokens],
        autojunk=False,
    )
    merged: list[str] = []
...
def request_consensus(
    ...
    for num_predict in (4096, 8192):
        ...
        if valid_consensus(consensus, qwen, whisper):
            return consensus, f"ollama:{model}"
        if body.get("done_reason") != "length":
            break
    return mechanical_merge(qwen, whisper), "ollama-mechanical-fallback"
...
def main() -> int:
    ...
    print(f"Ollama: {len(existing)} complete, {len(pending)} pending", flush=True)
    if not pending:
        return 0
        ...
        for chunk in pending:
            ...
            if lexical_words(q_text) == lexical_words(w_text):
                ...
                try:
                    ...
                    consensus, method = mechanical_merge(q_text, w_text), "ollama-error-fallback"
            record = {
                **chunk,
